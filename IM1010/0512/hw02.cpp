...
void stimulation(EventQueue events)
{
	...
	LinkedQueue<Customer> customers[3];
	bool available[3] = {1 , 1 , 1};
	int line[3] = {0};
	while (!events.isEmpty()){
		...
		if(newEvent.getType() == 0){
			...
			}//choose which bankteller to go
			line[togo]++;
			totalC++;
			cout << "Processing an arrival event at time: " << currentTime << "\n";
			if(available[togo] == true){
				Event departure(true , currentTime + newEvent.getLength() , togo);
				events.enqueue(departure);
				available[togo] = false;
			}
			else{
				Customer newCustomer(newEvent.getTime() , newEvent.getLength());
				customers[togo].enqueue(newCustomer);
			}
		}
		else{
			cout << "Processing a departure event at time: " << currentTime << "\n"; 
			if(!customers[newEvent.getLength()].isEmpty()){
				Customer c = customers[newEvent.getLength()].peekFront();
				customers[newEvent.getLength()].dequeue();
				WaitT += currentTime - c.getTime();
				Event departure(1 , currentTime + c.getLength() , newEvent.getLength());
				events.enqueue(departure);
			}
			else {
				available[newEvent.getLength()] = true;
			}
			line[newEvent.getLength()]--; 
		}
	} // end while
